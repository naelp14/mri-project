...
cg_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/cg_latent.csv") %>%
  dplyr::select(c(latent_1, latent_2))
cg_set <- data %>%
...
csf_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/csf_latent.csv") %>%
  dplyr::select(c(latent_1, latent_2))
csf_set <- data %>%
  mutate(
...
csf_data <- cbind(csf_set, csf_data)
...
pet_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/pet_latent.csv")
pet_data$cluster <- factor(pet_data$cluster, 
                           levels = c(0, 1, 2, 3), 
                           labels = c("Stable", "Mild Progression", "Moderate Progression", "Severe Progression"))
...
risk_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/risk_latent.csv") %>%
  dplyr::select(c(latent_1, latent_2))
risk_set <- data %>%
  select(c(AGE, PTGENDER, APOE4, PTEDUCAT, cluster)) %>%
  mutate(PTGENDER = ifelse(PTGENDER == "Male", 1, 0))

risk_data <- cbind(risk_set, risk_data)
...
mri_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/mri_latent.csv") %>%
  dplyr::select(c(latent_1, latent_2))
mri_set <- data %>%
  mutate(
...
mri_data <- cbind(mri_set, mri_data) %>% drop_na()
...
mri_risk_data <- read.csv("/Users/nathaniel.putera/Projects/UCPH/MRI-Project/mri_risk_latent.csv") %>%
  dplyr::select(c(latent_1, latent_2))
mri_risk_set <- data %>%
  mutate(
    PTGENDER = ifelse(PTGENDER == "Male", 1, 0),
    Ventricles = (Ventricles / ICV),
    Hippocampus = (Hippocampus / ICV),
    WholeBrain = (WholeBrain / ICV),
    Entorhinal = (Entorhinal / ICV),
...
mri_risk_data <- cbind(mri_risk_set, mri_risk_data) %>% drop_na()
...
# AUC
auc_data <- data.frame(
  Model = rep(c("cg_xgb", "csf_xgb", "mri_xgb", "risk_xgb", "mri_risk_xgb"), each = 4),
  Class = rep(c("Class_0", "Class_1", "Class_2", "Class_3"), times = 5),
  AUC = c(cg_xgb$auc, csf_xgb$auc, mri_xgb$auc, risk_xgb$auc, mri_risk_xgb$auc)
)
